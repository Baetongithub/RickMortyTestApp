...
import com.example.rickmortytestapp.data.mappers.toLocationEntity
import com.example.rickmortytestapp.data.remote.RickMortyAPI

@ExperimentalPagingApi
class LocationMediatorSource(
    private val rickMortyApi: RickMortyAPI,
    private val rickMortyDatabase: RickMortyDatabase
) : RemoteMediator<Int, ResultLocationEntity>() {

    private val rickMortyDao = rickMortyDatabase.getLocationDao()
    private val remoteKeysDao = rickMortyDatabase.getRemoteKeysDao()

    override suspend fun load(loadType: LoadType, state: PagingState<Int, ResultLocationEntity>): MediatorResult {
        return try {
            ...
            val response = rickMortyApi.getLocation(page = currentPage)
            ...
            rickMortyDatabase.withTransaction {
                if (loadType == LoadType.REFRESH) {
                    rickMortyDao.deleteAllLocation()
    ...
    private suspend fun getClosestRemoteKeyToCurrentPosition(
        state: PagingState<Int, ResultLocationEntity>
    ): RickMortyRemoteKeys? {
